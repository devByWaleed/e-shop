...
const AllEvents = () => {
    ...
    const { shopEvents, eventLoading, eventError } = useSelector((state) => state.event);
    ...
    const handleDelete = async (e, id) => {
        ...
        if (window.confirm("Are you sure you want to delete this product?")) {
            try {
                ...
                const { data } = await axios.delete(`/api/delete-event/${id}`)
        ...
        <section className='w-full max-w-7xl mx-auto p-4 md:p-6'>
            <div className='bg-white border rounded-xl text-sm shadow-sm border-light-border overflow-hidden'>
                ...
                {eventLoading ? (
                    ...
                    <div className='p-12 text-center text-secondary'>{eventError}</div>
                ) : events.length === 0 ? (
                    ...
                    <div className='divide-y divide-light-border max-h-[75vh] overflow-y-auto'>
                        {events.map((item) => {
                            ...
                            return (
                                <div
                                    key={item._id}
                                    onClick={() => navigate(`/events/${categorySlug}/${item._id}`)}
