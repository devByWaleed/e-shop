    ...
    return (
        <section onClick={() => { navigate(`/products/${categorySlug}/${product._id}`); scrollTo(0, 0); }} key={index} className="border border-zinc-200 hover:border-zinc-300 transition-colors rounded-xl p-4 flex flex-col w-72 md:w-50 relative cursor-pointer">
            ...
            <div className="flex items-center gap-2 px-2 mt-auto">
                ...
                <span className="text-xs font-bold text-secondary line-through">${product.originalPrice
                }</span>
                <span className="text-sm p-1 rounded text-accent">{product.soldOut} sold</span>
            </div>
        </section>
    )
}
