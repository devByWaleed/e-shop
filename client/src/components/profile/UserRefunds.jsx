...
const UserRefunds = () => {
    ...
    // FIX: Normalize strings by trimming spaces and lowercasing them to prevent strict string mismatch issues
    const eligibleOrders = orders 
        ? orders.filter((item) => item.status && item.status.trim().toLowerCase() === "processing refund") 
        : [];
        ...
        <div className="w-full bg-white border border-gray-200/80 rounded-2xl p-4 md:p-8 shadow-sm">
            ...
            {eligibleOrders.length === 0 ? (
                <div className="w-full flex flex-col items-center justify-center py-12 text-center">
                    <p className="text-sm text-gray-500 font-medium">No active refund requests found.</p>
                </div>
            ) : (
                <>
                    {/* Desktop Table View */}
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-50 text-sm text-gray-700">
                                {eligibleOrders.map((refund) => (
                                    <tr key={refund._id} className="hover:bg-gray-50/40 transition-colors group">
                                        <td className="py-4 px-6 font-mono text-xs text-gray-500 max-w-45 truncate">
                                            {refund._id}
                                        </td>
                                        
                                        <td className="py-4 px-4 font-mono text-xs text-gray-500 max-w-45 truncate">
                                            {refund._id}
                                        </td>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:hidden">
                        {eligibleOrders.map((refund) => (
                            <div key={refund._id} className="bg-gray-50/50 border border-gray-100 rounded-xl p-4 flex flex-col gap-2">
                                <div className="flex justify-between items-center">
                                    <span className={`text-xs font-medium px-2.5 py-1 rounded-md border ${getStatusStyle(refund.status)}`}>
                                        {refund.status}
                                    </span>
                                    <span className="text-sm font-bold text-gray-900">US$ {refund.totalPrice}</span>
                                </div>
