...
const SellerChatPage = () => {
    ...
    const isSellerView = Boolean(seller?._id);
    ...
    useEffect(() => {
        const fetchChatDetailsAndMessages = async () => {
            ...
            try {
                ...
                // FIXED: Get conversation by ID, not by member ID
    ...
    useEffect(() => {
        const fetchReceiverInfo = async () => {
            ...
            // id-prefix fallback, and skip the network call (and its fallback) entirely.
            ...
            try {
                let endpoint;
                if (isSellerView) {
                    // If seller is logged in, receiver is a buyer (user)
                    endpoint = `/api/user/user-info/${receiverId}`;
                } else {
                    // If user is logged in, receiver is a seller
                    endpoint = `/api/seller/get-seller-info/${receiverId}`;
                }

                const { data } = await axios.get(endpoint);
                if (data.success) {
                    const info = data.user || data.seller;
                    // Properly handle avatar URL
                ...
                try {
                    // Fallback: try to get seller info without auth
        ...
        fetchReceiverInfo();
    }, [receiverId, isSellerView, conversationData]);
