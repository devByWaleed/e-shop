...
import { getProductsAction } from '../../redux/actions/adminAction';
...
const AdminProducts = () => {
    ...
    const { allProducts, dataLoading } = useSelector((state) => state.admin);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Total Products: <span className="font-semibold text-gray-700">{allProducts.length}</span>
                    </p>
                    <button
                        onClick={() => navigate(-1)}
                                ...
                                {filteredProducts.map((product) => {
                                    ...
                                    return (
                                        <tr key={product._id} className="hover:bg-gray-50 transition-colors">
                                            <td title={product._id} className="px-6 py-4 text-sm font-mono text-gray-600 truncate">
                                            ...
                                            <td title={product.shop.name} className="truncate px-6 py-4 text-sm text-gray-600">
                                                <img
                                                    className='w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-semibold text-sm'
                                ...
                                <div key={product._id} className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow">
                                    <div className="flex items-start justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 font-mono">{truncateId(product._id)}</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="mt-3 grid grid-cols-2 gap-2">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-700 truncate">{product.shop.name}</p>
                                        </div>
